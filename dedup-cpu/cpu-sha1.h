...
void CPU_sha1_kernel( unsigned char* data_tmp, unsigned int length_tmp,  unsigned int* md)
{
	...
	unsigned int i, j;
	//dataLen = 8192;//数据长度为8192个字节
	//dataLen=1000;

	unsigned char add_data[8300];
	int kk;
	long long tmp;
	...
	for(j = 0; j < dataLen; j += 64)//每次取64个字节，即16个整数的长度
	{
		a = H0;
		b = H1;
		c = H2;
	...
	a = temp;
		...
		int ct=0;
	//对当前的8KB的数据生成160bit（5＊4＊8bit,20个字节，5个整形数）的摘要
...
void sha1(unsigned int *offset, unsigned char *input, unsigned char *output, int num) 
{
	for(int index=0;index<num;index++)
		CPU_sha1_kernel(input+offset[index],offset[index+1]-offset[index],(unsigned int*)(output+index*FINGERPRINT_LEN));
	
}

void CPU_sha1(unsigned char *input,unsigned char *output,unsigned int *offset,unsigned int num,unsigned int len)
{
		offset[num]=len;
		sha1(offset,input,output,num);
}
